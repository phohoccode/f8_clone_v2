...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($check_stmt->num_rows > 0) {
    ...
    echo "<script>alert('Slug đã tồn tại. Vui lòng chọn slug khác.');</script>";
    exit();
  }
  ...
  // Thêm khóa học mới
  $stmt = $conn->prepare("INSERT INTO courses (id, title, description, objectives, slug, thumbnail_url, price) VALUES (?, ?, ?, ?, ?, ?, ?)");
  $stmt->bind_param("ssssssd", $id, $title, $description, $objectives_json, $slug, $thumbnail_url, $price);
  $stmt->execute();
  $stmt->close();
...
<div id="modal-insert-course"
  ...
  <div class="bg-white w-full max-w-md p-6 rounded shadow-lg relative max-h-[80vh] overflow-y-auto">
    ...
    <!-- Nút đóng -->
    <button onclick="document.getElementById('modal-insert-course').classList.add('hidden')"
